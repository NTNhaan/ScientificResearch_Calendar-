...
import { PackedEvent, ThemeProperties } from "../../types";
import { roundTo, triggerHaptic } from "../../untils";
...
const DragEditItem = ({
  ...
  const recalculateEvent = () => {
    const newLeftPosition =
      event.leftByIndex !== undefined
        ? event.leftByIndex + translateX.value
        : 0;
  ...
  const dragPositionGesture = Gesture.Pan()
    ...
    .onUpdate(({ translationX, translationY, absoluteX }) => {
      const initIndex =
        event.leftByIndex !== undefined ? event.leftByIndex / columnWidth : 0;
